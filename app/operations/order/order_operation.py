...
class OrderOperation:
    ...
    def create_order(
        ...
        machine_selections = cls._validate_machine_availability(request.machine_selections)
            ...
            for selection in machine_selections:
                ...
                if selection.add_ons:
                    ...
                    for add_on in selection.add_ons:
                        ...
                        if 'price' in add_on_dict and isinstance(add_on_dict['price'], Decimal):
    ...
    def cancel_order(
        ...
        if not order.can_be_cancelled:
            raise ValueError(f"Order with status {order.status.value} cannot be cancelled")
    ...
    def get_order_statistics(
        ...
        total_revenue = query.with_entities(func.sum(Order.total_amount)).scalar() or Decimal('0.00')
        ...
        if not store_id:  # Only if not filtering by specific store
            store_orders = query.with_entities(
                Order.store_id, 
                func.count(Order.id)
    ...
    @classmethod
    @with_db_session_classmethod
    def _validate_machine_availability(cls, db: Session, machine_selections: List[Any]) -> List[Any]:
    ...
    @classmethod
    @with_db_session_classmethod
    def _calculate_add_ons_price(cls, db: Session, add_ons: List[Dict[str, Any]]) -> Decimal:
