...
from app.schemas.order import CreateOrderDetailRequest, UpdateOrderDetailStatusRequest
...
class OrderDetailOperation:
    ...
    def create_order_detail(
        ...
        existing_detail = self.db_session.query(OrderDetail).filter(
            and_(
        ...
        if existing_detail:
            raise ValueError(f"Machine {request.machine_id} is already booked in this order")
    ...
    def get_order_detail_by_id(self, detail_id: uuid.UUID) -> Optional[OrderDetail]:
        ...
        """
        return self.db_session.query(OrderDetail).filter(
            and_(
    ...
    def cancel_order_detail(
        ...
        if not self._can_modify_order(order):
            raise ValueError(f"Order with status {order.status.value} cannot be modified")
    ...
    def get_machine_usage_history(
        ...
        ).order_by(OrderDetail.created_at.desc()).limit(limit).all()
    ...
    def get_machine_availability_status(self, machine_id: uuid.UUID) -> Dict[str, Any]:
        ...
        active_detail = self.db_session.query(OrderDetail).filter(
            and_(
                OrderDetail.machine_id == machine_id,
                OrderDetail.deleted_at.is_(None),
                OrderDetail.status.in_([OrderDetailStatus.NEW, OrderDetailStatus.IN_PROGRESS])
            )
        ).first()
        ...
        return {
            ...
            'reason': 'Machine is available' if machine.status == MachineStatus.IDLE else f'Machine status: {machine.status.value}',
