...
CONDITION_TYPE = ConditionType.AMOUNT_PER_TENANT


@PromotionConditionCheckerRegistry.register(CONDITION_TYPE)
class AmountPerTenantPromotionConditionChecker(BasePromotionConditionChecker):
    ...
    def check(
        ...
        """
        Check if tenant's total order amount matches the condition.
        
        For AMOUNT_PER_TENANT condition:
        ...
        This checks the total amount of all successful orders for the tenant.
        """
        ...
        if not self.db:
            raise ValueError("Database session is required for AMOUNT_PER_TENANT condition checker")
        ...
        return apply_operator(condition.operator, tenant_total_amount, condition_value)
